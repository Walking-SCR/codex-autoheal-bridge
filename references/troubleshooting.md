...
- **Antigravity Gemini/Claude returns `429 RESOURCE_EXHAUSTED` only for full Codex prompts**: the upstream is matching the exact Codex identity fingerprint. The 8318 router rewrites that sentence only for the Gemini and Claude Antigravity routes and forces their WebSocket attempts to HTTP; verify an `antigravity_prompt_rewrite` event, then retry after any provider cooldown.
- **Antigravity returns `503 No capacity available for model ...`**: the upstream model has no serving capacity at that moment. Wait for `Retry-After`/provider cooldown and retry; do not treat this as a local OAuth or prompt-rewrite failure.
- **`antigravity_account_action_required` from the 8318 router, or raw `503 auth_unavailable` with `403 VALIDATION_REQUIRED` in `last upstream error`**: Google has flagged account-level verification. Verified 2026-09-26: OAuth re-login (`login-validation`) does NOT clear this wall; the fix is opening the per-account `validation_url` (`accounts.google.com/signin/continue?...`) from the 403 body, completing the challenge in the browser with the exact Google account, then deleting that account's `.cds` file AND restarting CLIProxyAPI (cooldown is in-memory — deleting files alone lets the proxy keep serving the cached 403). Run the automated flow `antigravity_pool.py validation-fix --model <model> --apply --restart` (see [antigravity-validation.md](antigravity-validation.md)). Do not retry the model first, swap URLs between accounts, or silently send the task to another provider. If the post-fix probe returns 429, verification succeeded and only quota remains — wait for reset.
- **`invalid compaction capsule: unrecognized compaction capsule format` after switching models in a long task**: this is a provider boundary error, not an OAuth, quota, or retry problem. A GPT/OpenAI task may contain a GPT compaction capsule while the target Gemini/Claude Antigravity route only accepts `cpa-ag-compact-v1:`. The router defaults to `fail_closed`, returns `provider_switch_compaction_conflict` before the request reaches 8317, and marks the response `handoff_required=true` plus `choice_required=true`. After the user selects the target model in the native picker, run `bridge.py provider-switch --thread-id <id> --target-model <model> --output <path>` and ask them to choose `handoff` (recommended; open a new task with the generated packet) or `cancel` (keep the source task unchanged). Do not silently retry or select the degraded path. For a controlled test only, an explicit `CODEX_BRIDGE_PROVIDER_SWITCH_COMPACTION_MODE=drop_foreign` setting removes incompatible compaction items and `previous_response_id`, but may discard compressed context. Do not delete OAuth files or repeatedly retry the same task.
- **`404 Item with id 'rs_...' not found` or `store=false` after switching models**: the task carries provider-scoped Responses state (`previous_response_id`, `item_reference`, `rs_`/`msg_`/`fc_`/`fco_` items) from the previous provider. The router records the task's provider and defaults to `fail_closed`, returning `provider_switch_state_conflict` before 8317; use the native-picker `handoff`/`cancel` choice. Do not globally force `store=true`. A controlled maintenance test may set `CODEX_BRIDGE_PROVIDER_SWITCH_STATE_MODE=drop_foreign`, which intentionally drops reasoning/tool state while preserving ordinary messages.
