...
- **`invalid compaction capsule: unrecognized compaction capsule format` after switching models in a long task**: this is a provider boundary error, not an OAuth, quota, or retry problem. A GPT/OpenAI task may contain a GPT compaction capsule while the target Gemini/Claude Antigravity route only accepts `cpa-ag-compact-v1:`. The router defaults to `fail_closed`, returns `provider_switch_compaction_conflict` before the request reaches 8317, and marks the response `handoff_required=true`. Generate a neutral handoff with `bridge.py handoff --thread-id <id> --target-model <model>` and paste it into a new task. For a controlled degraded test only, set `CODEX_BRIDGE_PROVIDER_SWITCH_COMPACTION_MODE=drop_foreign`; the router removes incompatible compaction items and `previous_response_id`, preserves ordinary messages, and logs the dropped format. Do not delete OAuth files or repeatedly retry the same task.
