...
DEFAULT_TRANSPARENT_RUNTIME = DEFAULT_STATE_DIR / "codex-model-router.mjs"
DEFAULT_CATALOG_POLICY = SKILL_DIR / "policies" / "catalog.json"
DEFAULT_LAUNCH_AGENT = Path(
    "~/Library/LaunchAgents/com.zhijian.codex-cli-model-bridge-router.plist"
).expanduser()
TRANSPARENT_LAUNCH_LABEL = "com.zhijian.codex-cli-model-bridge-router"
LEGACY_TRANSPARENT_LAUNCH_AGENT = Path(
...
def launch_agent_source(node: Path, runtime: Path, helper: Path, transparent_url: str, upstream_url: str) -> str:
    ...
    payload = {
        ...
        "EnvironmentVariables": {
            ...
            "CODEX_BRIDGE_LISTEN_HOST": "127.0.0.1",
            "CODEX_BRIDGE_LISTEN_PORT": str(transparent.port or 8318),
            "CODEX_BRIDGE_CLIPROXY_BASE_URL": f"http://127.0.0.1:{upstream.port or 8317}",
            "CODEX_BRIDGE_DISABLE_GPT_WS": "true",
        },
        "RunAtLoad": True,
        "KeepAlive": True,
        "ProcessType": "Background",
        "ThrottleInterval": 5,
        "StandardOutPath": str(Path("~/Library/Logs/CodexModelRouter.stdout.log").expanduser()),
        "StandardErrorPath": str(Path("~/Library/Logs/CodexModelRouter.stderr.log").expanduser()),
    }
...
def start_detached_proxy(
    ...
    env["CODEX_BRIDGE_LISTEN_HOST"] = "127.0.0.1"
    env["CODEX_BRIDGE_LISTEN_PORT"] = str(transparent.port or 8318)
    env["CODEX_BRIDGE_CLIPROXY_BASE_URL"] = f"http://127.0.0.1:{upstream.port or 8317}"
    env["CODEX_BRIDGE_DISABLE_GPT_WS"] = "true"
...
def cmd_add_model(args: argparse.Namespace) -> None:
    ...
    provider_name = args.provider_name.strip() if getattr(args, "provider_name", None) else re.sub(r'[^a-zA-Z0-9_-]', '', slug.split("-")[0].lower()) or "custom"
    ...
    # MiniMax special correction: if given anthropic endpoint, use standard v1 for OpenAI compatibility
    if "api.minimaxi.com/anthropic" in base_url:
        base_url = "https://api.minimaxi.com/v1"
    elif "api.minimax.chat" in base_url and not base_url.endswith("/v1"):
        base_url = f"{base_url}/v1"
    elif "api.minimaxi.com" in base_url and not base_url.endswith("/v1"):
        base_url = f"{base_url}/v1"

    probe_url = f"{base_url}/chat/completions"
    probe_payload = {
        "model": model_name,
    ...
    probe_headers = {
        ...
        "Content-Type": "application/json",
        "User-Agent": "Codex-Autoheal-Bridge/2.0"
    }

    try:
        req = urllib.request.Request(probe_url, data=json.dumps(probe_payload).encode("utf-8"), headers=probe_headers)
        with urllib.request.urlopen(req, timeout=15) as resp:
            _ = resp.read()
    except urllib.error.HTTPError as e:
        err_msg = e.read().decode("utf-8")[:300]
        emit({"status": "blocked", "error": f"Preflight probe failed at {probe_url} (HTTP {e.code}): {err_msg}"}, 2)
        return
    except Exception as e:
        emit({"status": "blocked", "error": f"Preflight connection failed at {probe_url}: {e}"}, 2)
        return

    # Stream termination check: detect if provider omits data: [DONE]
    ...
    has_done = False
    try:
        stream_req = urllib.request.Request(probe_url, data=json.dumps(stream_probe_payload).encode("utf-8"), headers=probe_headers)
        with urllib.request.urlopen(stream_req, timeout=15) as s_resp:
            for line in s_resp:
                if b"[DONE]" in line:
                    has_done = True
                    break
    ...
    clean_host_and_path = base_url.replace("https://", "").replace("http://", "").rstrip("/")
    configured_base_url = base_url if has_done else f"http://127.0.0.1:8318/__sse_shim/{clean_host_and_path}"
    ...
    result = {
        ...
        "stream_termination": "standard [DONE]" if has_done else "normalized (via 8318 SSE shim)",
        "provider_name": provider_name,
        "preflight_probe": "passed (HTTP 200)",
    }
    ...
    if proxy_config_path.exists():
        content = proxy_config_path.read_text(encoding="utf-8")
        if f'alias: "{slug}"' not in content and f"alias: '{slug}'" not in content and f'alias: {slug}' not in content:
            backup_path = backup(proxy_config_path)
            result["proxy_config_backup"] = str(backup_path)

            new_block = f"""  - name: "{provider_name}"
    base-url: "{configured_base_url}"
    api-key-entries:
      - api-key: "{api_key}"
    models:
      - name: "{model_name}"
...
"""
            ...
            result["proxy_config_updated"] = "already present"
    ...
    if catalog_path.exists():
        ...
        if not template and models_list:
            template = models_list[-1]

        if template:
            new_m = copy.deepcopy(template)
            new_m["slug"] = slug
            new_m["display_name"] = display_name
            new_m["description"] = f"{display_name} model via {provider_name} local bridge."
            new_m["priority"] = 58
            new_m["visibility"] = "list"
            new_m["supported_in_api"] = True

            models_list = [m for m in models_list if m.get("slug") != slug]
            models_list.append(new_m)
            catalog_data["models"] = models_list

            catalog_backup = backup(catalog_path)
            result["catalog_backup"] = str(catalog_backup)
            atomic_write(catalog_path, json.dumps(catalog_data, indent=2, ensure_ascii=False) + "\n", 0o644)
            result["catalog_updated"] = True
            result["total_catalog_models"] = len(models_list)
    ...
    bridge_key = ""
    try:
        bridge_key = read_client_key(Path(args.helper).expanduser())
    except Exception:
        pass

    if bridge_key:
        test_url = "http://127.0.0.1:8318/v1/responses"
        test_data = json.dumps({"model": slug, "input": "ping"}).encode("utf-8")
        test_req = urllib.request.Request(test_url, data=test_data, headers={
            "Authorization": f"Bearer {bridge_key}",
            "Content-Type": "application/json"
        })
        try:
            with urllib.request.urlopen(test_req, timeout=25) as test_resp:
                result["router_test"] = f"passed (HTTP {test_resp.status})"
        except Exception as e:
            result["router_test"] = f"warning: {e}"
...
def cmd_list_models(args: argparse.Namespace) -> None:
    ...
    if not catalog_path.exists():
        emit({"status": "blocked", "error": f"Catalog not found: {catalog_path}"}, 2)
        return
    with open(catalog_path, encoding="utf-8") as f:
        catalog_data = json.load(f)
...
def parser() -> argparse.ArgumentParser:
    ...
    add_m = sub.add_parser("add-model")
    add_m.add_argument("--model", required=True, help="Model ID or name, e.g. MiniMax-M3, qwen-2.5-coder")
    add_m.add_argument("--base-url", required=True, help="OpenAI-compatible base URL")
    add_m.add_argument("--api-key", required=True, help="API Key")
